...
class DeepLab(nn.Module):
    ...
    def forward(self, data, hhh, lll=None):
        b, c, h, w = data.shape
        blocks, merges = self.backbone(data, hhh)
        ppp, aux_fm = self.head(merges)
        ppp = F.interpolate(ppp, size=(h, w), mode='bilinear', align_corners=True)
        if lll is not None:
            loss = self.criterion(ppp, lll)
            loss2 = self.criterion(aux_fm, lll)
            return loss, loss2
        return ppp
...
class Head(nn.Module):
    ...
    def forward(self, f_list):
        f = f_list[-1]
        feat_8x = F.interpolate(f, scale_factor=2, mode='bilinear', align_corners=True)  # 8x
