...
class DualBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1,
        ...
        self.hha_bn1 = norm_layer(planes, eps=bn_eps, momentum=bn_momentum)
        self.hha_conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,padding=1, bias=False)
        self.hha_bn2 = norm_layer(planes, eps=bn_eps, momentum=bn_momentum)
        self.hha_conv3 = nn.Conv2d(planes, planes * self.expansion, kernel_size=1,bias=False)
        self.hha_bn3 = norm_layer(planes * self.expansion, eps=bn_eps,momentum=bn_momentum)
        self.hha_relu = nn.ReLU(inplace=inplace)
        self.hha_relu_inplace = nn.ReLU(inplace=True)
        self.hha_downsample = downsample
        self.stride = stride
...
class DualResNet(nn.Module):
    def __init__(self, block, layers, norm_layer=nn.BatchNorm2d, bn_eps=1e-5,
        ...
        self.pams = nn.ModuleList([
            PAM(in_planes=256, out_planes=256, bn_momentum=bn_momentum),
            PAM(in_planes=512, out_planes=512, bn_momentum=bn_momentum),
            PAM(in_planes=1024, out_planes=1024, bn_momentum=bn_momentum),
            PAM(in_planes=2048, out_planes=2048, bn_momentum=bn_momentum)
        ])
    ...
    def forward(self, x1, x2):
        ...
        merges = []
        x = self.layer1(x)
        x, merge = self.pams[0](x)
        blocks.append(x)
        merges.append(merge)

        x = self.layer2(x)
        x, merge = self.pams[1](x)
        blocks.append(x)
        merges.append(merge)

        x = self.layer3(x)
        x, merge = self.pams[2](x)
        blocks.append(x)
        merges.append(merge)

        x = self.layer4(x)
        x, merge = self.pams[3](x)
        blocks.append(x)
        merges.append(merge)
