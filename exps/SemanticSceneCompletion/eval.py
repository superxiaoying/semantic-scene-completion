...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        ...
        name = data['fn']
        pp, ps, pe = self.eval_ssc(iii, ttt, mmm, device)

        results_dict = {'pp':pp, 'll':lll, 'ww':www,
                        'name':name, 'mm':mmm}
        return results_dict
    ...
    def compute_metric(self, results):
        ...
        for d in results:
            ...
            ooo = (mmm == 307200) & (www > 0) & (flat_lll != 255)
    ...
    def eval_ssc(self, iii, ttt, mmm, edge_output, device=None):
        input_data = self.process_image(iii, crop_size=None)
        sc, bsc, esc = self.val_func_process_ssc(input_data, ttt, mmm, device)
    ...
    def val_func_process_ssc(self, ddd, ttt, mmm, device=None):
        ddd = np.ascontiguousarray(ddd[None, :, :, :], dtype=np.float32)
        ddd = torch.FloatTensor(ddd).cuda(device)

        mmm = np.ascontiguousarray(mmm[None, :], dtype=np.int32)
        mmm = torch.LongTensor(mmm).cuda(device)

        ttt = np.ascontiguousarray(ttt[None, :], dtype=np.float32)
        ttt = torch.FloatTensor(ttt).cuda(device)

        with torch.cuda.device(ddd.get_device()):
            self.val_func.eval()
            self.val_func.to(ddd.get_device())
            with torch.no_grad():
                sc, ssc, sssc = self.val_func(ddd, mmm, ttt, None)
                sc = sc[0]
                sc = torch.exp(sc)
        return sc, ssc, sssc
